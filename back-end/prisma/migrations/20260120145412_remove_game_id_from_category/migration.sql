...
CREATE TABLE `Game` (
    ...
    `status` ENUM('DRAFT', 'PUBLISHED', 'SOLD') NOT NULL DEFAULT 'PUBLISHED',
...
CREATE TABLE `_GameCategories` (
    ...
    UNIQUE INDEX `_GameCategories_AB_unique`(`A`, `B`),
    INDEX `_GameCategories_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `Game` ADD CONSTRAINT `Game_sellerId_fkey` FOREIGN KEY (`sellerId`) REFERENCES `User`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
